...
COVS = ['onpromotion']+holiday
CATS = ['onpromotion']+holiday
...
def forecast_time_series(time_series_i):
    ...
    try:
        for first_test,end_test in zip(test_dates,test_end_dates):
            ...
            past_data = data.query('date < @first_test & time_series == @time_series_i').set_index('date').asfreq('D').fillna(0).astype(float)
            future_covs = data.query('date >= @first_test & date <= @end_test & time_series == @time_series_i').set_index('date').asfreq('D').fillna(0).astype(float)
            ...
            if MODEL == 'arima':
                model =  AutoARIMA(seasonal=True,season_length=SEASON_LENGTH, allowdrift=True,allowmean=True,num_cores = -1,approximation=False)
            elif MODEL == 'ets':
            ...
            while max_pred < end_test:
                ...
                while first_test not in valid_dates.values:    # ADD: also check if max_pred is in valid_dates
                    first_test = first_test + pd.Timedelta(days=1)

                past_data = data.query('date < @first_test & time_series == @time_series_i').set_index('date').asfreq('D').fillna(0).astype(float)
                future_covs = data.query('date >= @first_test & date <= @end_test & time_series == @time_series_i').set_index('date')[covs].asfreq('D').fillna(0).astype(float)
                if HORIZON == 12:
                    past_data = past_data[past_data.index.dayofweek != 6]
                    future_covs = future_covs[future_covs.index.dayofweek != 6]
                max_pred = first_test + pd.Timedelta(days=13)
...
#     try:
#         results.append(forecast_time_series(ts))

#     except:
#         print(f'Error with {ts}')
#         continue
