...
first_run = True
for train_end,test_end_date in zip(train_ends,test_end_dates):  ####NEW

    df_train = df[df['time_idx'] <= train_end - val_length]
    df_val = df[(df['time_idx'] > train_end - val_length - enc_len) & (df['time_idx'] <= train_end)]
    df_test = df[(df['time_idx'] > train_end - enc_len) & (df['date'] < test_end_date)]  ###NEW  should actualy be <, this way we have one more test date than we wanted...will be corrected later without any harm
    ...
    for k in covs:
        if k == 'attention':
            ...
            for step in range(raw.output[k].shape[2]):
                preds_df[f'att_{-raw.output[k].shape[2]+step}'] = raw.output[k][:,:,step].flatten().cpu()
        else:
