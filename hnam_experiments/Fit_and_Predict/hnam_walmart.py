...
for train_end,test_end_date in zip(train_ends,test_end_dates):
    ...
    trainer = pl.Trainer(
        ...
        accelerator=ACC,
        devices=DEVICES,  # Specify the number of GPUs you want to use
        strategy=STRATEGY,
    ...
    # preds_df = pd.DataFrame(index.values.repeat(dec_len * n_quantiles, axis=0),columns=index.columns)s
