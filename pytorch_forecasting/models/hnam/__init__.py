...
class HNAM(BaseModelWithCovariates):
    ...
    def forward(self, x: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        x_level, trend_emb = self.trend_block(trend_query,past_key_values,trend_normal)
        ...
        for i,feature in enumerate(self.hparams.causal):
            causal_decoder[feature] = self.dtf(x_encoded,self.hparams.causal[:i+1]+self.hparams.base,stack=True).sum(axis=2)[:,max_encoder_length:,:] + trend_emb
