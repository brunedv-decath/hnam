...
class Trend(nn.Module):
    def __init__(self, q_in, k_in, trend_emb,output_size=1, bias=False):
        super().__init__()

        self.ln_q = LayerNorm(q_in, bias=bias)
        self.ln_k = LayerNorm(k_in, bias=bias)
        self.query_proj = nn.Linear(q_in, trend_emb, bias=False)
        self.key_proj = nn.Linear(k_in, trend_emb, bias=False)
...
class ApplyAttention(nn.Module):
        ...
        def __init__(self,cov_emb,bias):
            super().__init__()
            self.ln = LayerNorm(cov_emb, bias=bias)
            self.proj_v = nn.Linear(cov_emb, cov_emb, bias=bias)
            self.proj_back = MLP(cov_emb,cov_emb,factor=2,dropout=0.3,bias=bias)
    
        def forward(self,v,att):
            #print(att.shape)# -> 64,2,12,42
            _,_,C = v.size()
...
#             super().__init__()
#             self.ln = LayerNorm(cov_emb, bias=bias)
#             self.proj_v = MLP(cov_emb,cov_emb,factor=2,dropout=0.3,bias=bias)
...
class HNAM(BaseModelWithCovariates):
    def __init__(
        ...
        **kwargs,
        ...
        self.trend_block = Trend(q_in = cov_emb,
                                 k_in = cov_emb,
                                 trend_emb = trend_emb,
        ...
        self.attention_apply = ApplyAttention(cov_emb=cov_emb,bias=bias)
    ...
    def forward(self, x: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        for c in self.hparams.causal:
            # keys_values = torch.concat([known_past,causal_decoder[c]],dim=1)
            # causal_decoder[c] = self.attention_block(causal_decoder[c],keys_values)
            if ATTENTION:
